 ...
 *   ✓ Actualiza el nombre en el topbar sin recargar (actualizarUsuario)
 */

import React, { useEffect, useState } from 'react';
import { User, Phone, Lock, Eye, EyeOff, CheckCircle, AlertCircle, RefreshCw } from 'lucide-react';
import { authApi, perfilApi, equiposApi, type PerfilCompleto, type Equipo } from '@/lib/api';
import { useAuth } from '@/context/AuthContext';
import styles from '@/styles/pages/dashboard/dashboard.module.css';

function Alerta({ tipo, mensaje }: { tipo: 'ok' | 'err'; mensaje: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0.75rem 1rem', borderRadius: '10px', background: tipo === 'ok' ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)', border: `1px solid ${tipo === 'ok' ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}`, color: tipo === 'ok' ? '#4ade80' : '#f87171', fontSize: '0.85rem', lineHeight: 1.5 }} role="alert">
...
const btnPrimary: React.CSSProperties = { background: 'linear-gradient(135deg, var(--primary), var(--secondary))', color: '#000', border: 'none', padding: '0.8rem 1.6rem', borderRadius: '12px', fontWeight: 700, cursor: 'pointer', fontSize: '0.9rem' };
const btnSecondary: React.CSSProperties = { background: 'var(--btn-secondary-bg)', color: 'var(--btn-secondary-text)', border: '1px solid var(--btn-secondary-border)', padding: '0.8rem 1.6rem', borderRadius: '12px', fontWeight: 600, cursor: 'pointer', fontSize: '0.9rem' };

function iniciales(nombre: string) { return nombre.trim().split(' ').slice(0, 2).map(p => p[0]?.toUpperCase() ?? '').join(''); }
const ROL_COLOR: Record<string, string> = { admin: '#a855f7', entrenador: '#3b82f6', recepcionista: '#06b6d4', socio: '#06b6d4' };

export default function PerfilPage() {
  // HU-03: acceso a actualizarUsuario para reflejar cambios en el topbar
  ...
  useEffect(() => {
    ...
    if (local) {
      ...
      setPerfil({ id_usuario: local.id_usuario, nombre: local.nombre, correo: local.correo, identificacion: '', telefono: null, estado: true, rol: { id_rol: 0, nombre: local.rol } });

      // HU-20: Fetch availability for socios
      if (local.rol.toLowerCase() === 'socio') {
    ...
    perfilApi.obtener()
      .then((data) => { setPerfil(data); setNombre(data.nombre); setTelefono(data.telefono ?? ''); setErrorCarga(''); })
      .catch((e: unknown) => {
        const msg = e instanceof Error ? e.message : 'Error al cargar el perfil';
        if (msg.includes('401') || msg.toLowerCase().includes('unauthorized')) { authApi.logout(); window.location.href = '/login'; return; }
        if (local) setErrorCarga('⚠ Mostrando datos locales. No se pudo sincronizar con el servidor.');
        else setErrorCarga(msg);
      })
  ...
  const handleGuardarDatos = async (e: React.FormEvent) => {
    ...
    try {
      // Pasa el callback — cuando el servidor confirma, actualiza el topbar al instante
      const actualizado = await perfilApi.actualizar(
        { nombre: nombre.trim(), telefono: telefono.trim() || undefined },
        (nuevoNombre) => actualizarUsuario({ nombre: nuevoNombre }),
      );
      setPerfil(actualizado);
      setNombre(actualizado.nombre);
      setTelefono(actualizado.telefono ?? '');
      setAlertaDatos({ tipo: 'ok', msg: 'Datos actualizados correctamente. El nombre en el topbar ya refleja el cambio.' });
    } catch (err: unknown) {
  ...
  const handleCambiarPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAlertaPass({ tipo: 'err', msg: msg.toLowerCase().includes('actual') || msg.toLowerCase().includes('incorrecta') ? 'La contraseña actual no es correcta.' : msg });
    } finally { setGuardandoPass(false); }
  };

  if (cargando && !perfil) return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '300px', flexDirection: 'column', gap: '1rem', color: 'var(--text-faint)' }}>
  ...
  if (errorCarga && !perfil) return <div className={styles.container}><Alerta tipo="err" mensaje={errorCarga} /></div>;
  ...
  return (
    <div className={styles.container} style={{ maxWidth: '760px' }}>
      ...
      <div className="glass" style={{ borderRadius: '20px', padding: '2rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1 }}>
          <h2 style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--text-main)', marginBottom: '4px' }}>{perfil?.nombre}</h2>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '6px' }}>{perfil?.correo}</p>
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <span style={{ padding: '2px 12px', borderRadius: '20px', fontSize: '0.78rem', fontWeight: 600, background: `${rolColor}22`, color: rolColor, border: `1px solid ${rolColor}44` }}>{rolNombre.charAt(0).toUpperCase()}{rolNombre.slice(1)}</span>
        ...
        {perfil?.identificacion && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '0.8rem', color: 'var(--text-faint)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.25rem' }}>
        ...
        <div className="glass" style={{ borderRadius: '18px', padding: '1.75rem' }}>
          <h3 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '1.25rem' }}>
            <User size={18} color="var(--primary)" /> Datos personales
          </h3>
          <p style={{ fontSize: '0.8rem', color: 'var(--text-faint)', marginBottom: '1.25rem' }}>
            Puedes editar tu nombre y teléfono. El correo e identificación solo pueden modificarlos los administradores.
          </p>
          <form onSubmit={handleGuardarDatos} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div><label style={labelStyle}>Correo electrónico</label><input type="email" value={perfil?.correo ?? ''} readOnly style={{ ...inputStyle, opacity: 0.45, cursor: 'not-allowed' }} /></div>
              <div><label style={labelStyle}>Identificación</label><input type="text" value={perfil?.identificacion ?? ''} readOnly style={{ ...inputStyle, opacity: 0.45, cursor: 'not-allowed' }} /></div>
            </div>
        ...
        <div className="glass" style={{ borderRadius: '18px', padding: '1.75rem' }}>
          ...
          <p style={{ fontSize: '0.8rem', color: 'var(--text-faint)', marginBottom: '1.25rem' }}>
            Debes ingresar tu contraseña actual para establecer una nueva. Mínimo 8 caracteres.
          </p>
          <form onSubmit={handleCambiarPassword} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            ] as const).map(({ label, val, set, ver, setVer, ph }) => (
              <div key={label}>
                <label style={labelStyle}>{label}</label>
                <div style={{ position: 'relative' }}>
                  <input type={ver ? 'text' : 'password'} value={val} onChange={e => set(e.target.value)} placeholder={ph} style={{ ...inputStyle, paddingRight: '3rem' }} disabled={guardandoPass} />
                  <button type="button" onClick={() => setVer((p: boolean) => !p)} style={{ position: 'absolute', right: '0.9rem', top: '50%', transform: 'translateY(-50%)', background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}>
                    {ver ? <EyeOff size={16} /> : <Eye size={16} />}
                  </button>
                </div>
              </div>
            ))}
            {passConfirm && passNueva !== passConfirm && <p style={{ fontSize: '0.75rem', color: '#f87171', marginTop: '-8px' }}>Las contraseñas no coinciden</p>}
      ...
      {/* HU-20: Consulta de Disponibilidad para Socios */}
      {rolNombre.toLowerCase() === 'socio' && (
        <div style={{ marginTop: '1.5rem' }}>
          <h3 style={{ fontSize: '1.2rem', fontWeight: 600, color: 'var(--text-main)', marginBottom: '1rem' }}>Disponibilidad de Máquinas y Equipos</h3>
          {cargandoEquipos ? (
            <p style={{ color: 'var(--text-faint)' }}>Cargando disponibilidad...</p>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
              {equipos.map(eq => (
                <div key={eq.id_equipo} className="glass" style={{
                  borderRadius: '12px',
                  padding: '1rem',
                  borderLeft: `4px solid ${eq.estado === 'Operativo' || eq.estado === 'disponible' ? '#4ade80' : eq.estado === 'En Mantenimiento' ? '#facc15' : '#f87171'}`,
                  background: 'var(--subtle-bg)'
                }}>
                  <p style={{ fontWeight: 600, margin: 0, color: 'var(--text-main)' }}>{eq.nombre}</p>
                  <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '10px' }}>{eq.tipo}</p>
                  <span style={{
